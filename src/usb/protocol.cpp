...
struct usb_device_configuration {
    ...
    const char **local_descriptor_strings;
...
const char **local_descriptor_strings;
uint16_t local_descriptor_strings_len;
...
void usb_handle_string_descriptor(volatile usb_setup_packet *pkt) {
    ...
    if (i == 0) {
        ...
        len = usb_prepare_string_descriptor(dev_config.local_descriptor_strings[i - 1]);
    }
...
void initMode(USBConfiguration config) {
    ...
    local_descriptor_strings = config.descriptorStrings;
    local_descriptor_strings_len = config.descriptorStringsLen;
    ...
    dev_config = {
        ...
        .local_descriptor_strings = local_descriptor_strings,
        .endpoints = {
            {
                .descriptor = &ep0_out,
...
void usb_start_transfer_in_ep(uint8_t *buf, uint16_t len) {
    usb_start_transfer(usb_get_endpoint_configuration(ep_in_addr()), buf, len);
}

void usb_start_transfer_out_ep(uint8_t *buf, uint16_t len) {
    usb_start_transfer(usb_get_endpoint_configuration(ep_out_addr()), buf, len);
}

bool in_ep_available() {
    // AVAIL seems to mean, hey phy you can send this
    // FULL is for us, i.e full = we filled it, and it's cleared when sent
    return !(usb_dpram->ep_ctrl[0].in & USB_BUF_CTRL_FULL);
}